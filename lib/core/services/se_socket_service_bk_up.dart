...
class SeSocketService {
  ...
  static const Duration _maxReconnectDelay =
      Duration(minutes: 3); // Reduced from 5 to 3 minutes
  static const int _maxReconnectAttempts = 8; // Reduced from 10 to 8 attempts
  ...
  void _setupSocketEventHandlers() {
    ...
    // Listen for channel-based key exchange events: key_exchange:sessionId:request
    // This listens for KERs sent TO this user (where they are the recipient)
    print('🔌 SeSocketService: 🔧 Setting up KER request event listener...');
    print('🔌 SeSocketService: 🔍 Current session ID: $_currentSessionId');
    print(
        '🔌 SeSocketService: 🔍 Event name: key_exchange:${_currentSessionId}:request');
    print('🔌 SeSocketService: 🔍 Socket connected: ${_socket?.connected}');
    print('🔌 SeSocketService: 🔍 Socket ID: ${_socket?.id}');

    _socket!.on('key_exchange:${_currentSessionId}:request', (data) async {
    ...
    // Listen for channel-based key exchange events: key_exchange:sessionId:response
    print('🔌 SeSocketService: 🔧 Setting up KER response event listener...');
    print('🔌 SeSocketService: 🔍 Current session ID: $_currentSessionId');
    print(
        '🔌 SeSocketService: 🔍 Event name: key_exchange:${_currentSessionId}:response');

    _socket!.on('key_exchange:${_currentSessionId}:response', (data) async {
      try {
        final isSilent = (data['silent'] as bool?) ?? false;
    ...
    // Listen for channel-based user data exchange events: user_data_exchange:sessionId:data
    _socket!.on('user_data_exchange:${_currentSessionId}:data', (data) async {
      try {
        final isSilent = (data['silent'] as bool?) ?? false;
    ...
    // Listen for channel-based conversation created events: conversation_created:sessionId:confirmation
    _socket!.on('conversation_created:${_currentSessionId}:confirmation',
        (data) async {
      try {
        final isSilent = (data['silent'] as bool?) ?? false;
        final senderId = data['sender_id'] as String? ?? '';
        final conversationIdLocal =
            data['conversation_id_local'] as String? ?? '';
        final timestamp = data['timestamp'] as String? ?? '';

        if (senderId.isEmpty) {
          print(
              '🔌 SeSocketService: ❌ Invalid conversation created payload - missing sender_id');
          return;
        }

        print(
            '🔌 SeSocketService: 🆕 Conversation created confirmation received from $senderId');
        print(
            '🔌 SeSocketService: 🔍 Local conversation ID: $conversationIdLocal');
        print('🔌 SeSocketService: 🔍 Timestamp: $timestamp');

        // Notify the conversation created callback
        if (_onConversationCreated != null) {
          // Create a conversation object for the callback
          final conversationData = {
            'id': conversationIdLocal.isNotEmpty
                ? conversationIdLocal
                : 'conv_${DateTime.now().millisecondsSinceEpoch}',
            'participant1Id': _currentSessionId ?? '',
            'participant2Id': senderId,
            'createdAt': DateTime.now().toIso8601String(),
            'metadata': Map<String, dynamic>.from(data),
          };

          _onConversationCreated!(conversationData);
          print(
              '🔌 SeSocketService: ✅ Conversation created callback triggered');
        } else {
          print('🔌 SeSocketService: ⚠️ No conversation created callback set');
        }

        if (!isSilent) {
          if (AppStateService().isForeground) {
            UIService().showSnack('Conversation created successfully');
          } else {
            // Create notification for conversation creation
            createKeyExchangeNotification(
              type: 'conversation_created',
              ...
              senderName: 'Conversation Created',
              message:
                  'A conversation has been created with ${senderId.substring(0, 8)}...',
              metadata: Map<String, dynamic>.from(data),
            );
          }
        }
      } catch (e) {
        print('🔌 SeSocketService: ❌ Error handling conversation_created: $e');
      }
    });

    // Listen for channel-based key exchange revoked events: key_exchange:sessionId:revoked
    _socket!.on('key_exchange:${_currentSessionId}:revoked', (data) {
      try {
    ...
    // Listen for channel-based typing events: typing:sessionId:update
    _socket!.on('typing:${_currentSessionId}:update', (data) {
      try {
        final conversationId = data['conversationId'] as String? ?? '';
    ...
    // Listen for channel-based message status events: message:sessionId:acked
    _socket!.on('message:${_currentSessionId}:acked', (data) {
      try {
        final messageId = data['messageId'] as String? ?? '';
        final timestamp =
            data['timestamp'] as String? ?? DateTime.now().toIso8601String();
        ...
        // Note: Message delivery state updates are now handled by the channel-based socket system
        print(
            '🔌 SeSocketService: ℹ️ Message ack received - delivery state managed by channel socket');
      } catch (e) {
    ...
    // Listen for channel-based message status events: message:sessionId:delivered
    _socket!.on('message:${_currentSessionId}:delivered', (data) {
      try {
        final messageId = data['messageId'] as String? ?? '';
        final timestamp =
            data['timestamp'] as String? ?? DateTime.now().toIso8601String();
        ...
        // Note: Message delivery state updates are now handled by the channel-based socket system
        print(
            '🔌 SeSocketService: ℹ️ Message delivered - delivery state managed by channel socket');
      } catch (e) {
    ...
    // Listen for channel-based message status events: message:sessionId:read
    _socket!.on('message:${_currentSessionId}:read', (data) {
      try {
        final messageId = data['messageId'] as String? ?? '';
        final timestamp =
            data['timestamp'] as String? ?? DateTime.now().toIso8601String();
        ...
        // Note: Message delivery state updates are now handled by the channel-based socket system
        print(
            '🔌 SeSocketService: ℹ️ Message read - delivery state managed by channel socket');
      } catch (e) {
    ...
    // Listen for channel-based presence events: presence:sessionId:update
    _socket!.on('presence:${_currentSessionId}:update', (data) {
      try {
    ...
    // Listen for channel-based receipt events: receipt:sessionId:delivered
    _socket!.on('receipt:${_currentSessionId}:delivered', (data) {
      try {
        final messageId = data['messageId'] as String? ?? '';
        final fromUserId = data['fromUserId'] as String? ?? '';
        ...
        // Note: Message delivery state updates are now handled by the channel-based socket system
        print(
            '🔌 SeSocketService: ℹ️ Receipt delivered - delivery state managed by channel socket');
      } catch (e) {
    ...
    // Listen for channel-based receipt events: receipt:sessionId:read
    _socket!.on('receipt:${_currentSessionId}:read', (data) {
      try {
        final messageId = data['messageId'] as String? ?? '';
        final fromUserId = data['fromUserId'] as String? ?? '';
        final toUserId = data['toUserId'] as String? ?? '';
        ...
        // Note: Message delivery state updates are now handled by the channel-based socket system
        print(
            '🔌 SeSocketService: ℹ️ Receipt read - delivery state managed by channel socket');
      } catch (e) {
    ...
    // Listen for channel-based message events: message:sessionId:received
    _socket!.on('message:${_currentSessionId}:received', (data) {
      try {
        final messageId = data['messageId'] as String? ?? '';
        final fromUserId = data['fromUserId'] as String? ?? '';
        final conversationId = data['conversationId'] as String? ?? '';
        ...
        // Note: Delivery receipts are now handled by the channel-based socket system
        print(
            '🔌 SeSocketService: ℹ️ Delivery receipt - managed by channel socket');
      } catch (e) {
    ...
    print('🔌 SeSocketService: 📋 Event listeners configured:');
    print('🔌 SeSocketService:   - connect, disconnect, connect_error');
    print('🔌 SeSocketService:   - session_expired, session_invalid');
    print('🔌 SeSocketService:   - new_message');
    print('🔌 SeSocketService:   - key_exchange:${_currentSessionId}:request');
    print('🔌 SeSocketService:   - key_exchange:${_currentSessionId}:response');
    print('🔌 SeSocketService:   - key_exchange:${_currentSessionId}:revoked');
    print(
        '🔌 SeSocketService:   - user_data_exchange:${_currentSessionId}:data');
    print(
        '🔌 SeSocketService:   - conversation_created:${_currentSessionId}:confirmation');
    print('🔌 SeSocketService:   - presence:${_currentSessionId}:update');
    print('🔌 SeSocketService:   - typing:${_currentSessionId}:update');
    print('🔌 SeSocketService:   - message:${_currentSessionId}:acked');
    print('🔌 SeSocketService:   - message:${_currentSessionId}:status_update');
    print('🔌 SeSocketService:   - user_deleted');
    print('🔌 SeSocketService: 🔍 Current session ID: $_currentSessionId');
    print('🔌 SeSocketService: 🔍 Socket connected: ${_socket?.connected}');
    print('🔌 SeSocketService: 🔍 Socket ID: ${_socket?.id}');
  }
  ...
  void _scheduleReconnect() {
    ...
    // Use less aggressive reconnection with longer delays to reduce connection instability
    final baseDelay =
        Duration(seconds: 10); // Start with 10 seconds instead of 5
    final maxDelay = Duration(minutes: 3); // Cap at 3 minutes instead of 5

    final delay = Duration(
        seconds: (baseDelay.inSeconds * (1 << _reconnectAttempts))
            .clamp(baseDelay.inSeconds, maxDelay.inSeconds));
  ...
  void _startHeartbeat() {
    ...
    // Increase heartbeat frequency to 15 seconds for better connection health monitoring
    _heartbeatTimer = Timer.periodic(const Duration(seconds: 15), (timer) {
      if (_isConnected && _socket != null) {
        try {
          ...
          // Verify connection health after heartbeat
          if (!_socket!.connected) {
            print(
                '🔌 SeSocketService: ⚠️ Heartbeat detected connection loss, triggering reconnection');
            _isConnected = false;
            _connectionStateController.add(false);
            _scheduleReconnect();
          }
        } catch (e) {
          print('🔌 SeSocketService: ❌ Heartbeat failed: $e');
          // If heartbeat fails, connection might be broken
          _isConnected = false;
          _connectionStateController.add(false);
          _scheduleReconnect();
        }
  ...
  Future<bool> sendMessage({
    ...
    try {
      ...
      if (_isConnected) {
        // Use channel-based event format: message:recipientId:send
        emit('message:${recipientId}:send', messageData);
        print(
            '🔌 SeSocketService: ✅ Message sent immediately via new protocol using channel format');
        return true;
      } else {
        ...
        if (connected) {
          ...
          emit('message:${recipientId}:send', messageData);
          print(
              '🔌 SeSocketService: ✅ Message queued via socket after connection using channel format');
          return true;
        } else {
  ...
  Future<bool> sendMessageStatusUpdate({
    ...
    try {
      ...
      // Use channel-based event format: message:recipientId:status_update
      emit('message:${recipientId}:status_update', statusData);
      return true;
    } catch (e) {
  ...
  Future<bool> sendUserOnlineStatus(bool isOnline) async {
    try {
      ...
      // Use channel-based event format: presence:server:update (server-wide presence)
      emit('presence:server:update', statusData);
      print(
  ...
  Future<bool> sendKeyExchangeRequest({
    ...
    try {
      ...
      // If socket is connected, send immediately
      if (_isConnected) {
        // Use channel-based event format: key_exchange:recipientId:request
        emit('key_exchange:${recipientId}:request', keyExchangeData);
        print(
            '🔌 SeSocketService: ✅ Key exchange request sent immediately via socket using channel format');
      } else {
        ...
        if (connected) {
          ...
          emit('key_exchange:${recipientId}:request', keyExchangeData);
          print(
              '🔌 SeSocketService: ✅ Key exchange request queued via socket after connection using channel format');
        } else {
          // If connection fails, emit a queue event that the server should handle
  ...
  Future<bool> sendKeyExchangeResponse({
    ...
    try {
      ...
      // Use channel-based event format: key_exchange:recipientId:response
      emit('key_exchange:${recipientId}:response', response);
      return true;
    } catch (e) {
  ...
  Future<bool> revokeKeyExchangeRequest({
    ...
    try {
      ...
      // Use channel-based event format: key_exchange:recipientId:revoked
      emit('key_exchange:${recipientId}:revoked', revokeData);
      return true;
    } catch (e) {
