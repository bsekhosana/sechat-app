...
class SecureNotificationService {
  ...
  /// Send encrypted invitation notification with key exchange
  Future<bool> sendEncryptedInvitation({
    required String recipientId,
    required String senderName,
    ...
    try {
      ...
      // First, send an unencrypted key exchange request
      print(
          '🔒 SecureNotificationService: Initiating key exchange before sending invitation');
      final keyExchangeSuccess =
          await KeyExchangeService.instance.requestKeyExchange(recipientId);

      if (!keyExchangeSuccess) {
        print(
            '🔒 SecureNotificationService: Key exchange request failed, sending invitation with fallback key');
        // Generate a test key as fallback if key exchange fails
        if (!await KeyExchangeService.instance
            .hasPublicKeyForUser(recipientId)) {
          print(
              '🔒 SecureNotificationService: Using fallback key for $recipientId');
          await EncryptionService.generateTestKey(recipientId);
        }
      } else {
        print(
            '🔒 SecureNotificationService: Key exchange request sent successfully');
        // Wait for key exchange to complete with timeout
        final keyExchangeCompleted = await _waitForKeyExchange(recipientId);
        if (!keyExchangeCompleted) {
          print(
              '🔒 SecureNotificationService: Key exchange timeout, using fallback key');
          // Generate a test key as fallback if key exchange times out
          if (!await KeyExchangeService.instance
              .hasPublicKeyForUser(recipientId)) {
            await EncryptionService.generateTestKey(recipientId);
          }
          ...
          print(
              '🔒 SecureNotificationService: Key exchange completed successfully');
        }
      }
      ...
      final invitationData = {
        ...
        'timestamp': DateTime.now().millisecondsSinceEpoch,
        'key_exchange_initiated': keyExchangeSuccess,
      };
      ...
      // Send via notification service
      return await SimpleNotificationService.instance
          .sendEncryptedInvitationNotification(
        recipientId: recipientId,
        senderName: senderName,
      ...
      print(
          '🔒 SecureNotificationService: Error sending encrypted invitation: $e');
      return false;
    }
  }

  /// Send encrypted invitation response notification
  Future<bool> sendEncryptedInvitationResponse({
    required String recipientId,
    ...
    try {
      ...
      if (!keyExchangeSuccess) {
        print(
            '🔒 SecureNotificationService: Key exchange failed, cannot encrypt invitation response');
        return false;
      }
      ...
      // Send via notification service
      return await SimpleNotificationService.instance
          .sendEncryptedInvitationResponseNotification(
        recipientId: recipientId,
      ...
      print(
          '🔒 SecureNotificationService: Error sending encrypted invitation response: $e');
      return false;
    }
  }
